...
fn analyze_error_sizes() {
    ...
    println!("  serde_json::Error: {} bytes", size_of::<serde_json::Error>());
    ...
    println!("Expected enum size (variant + discriminant): ~{} bytes", max_variant_size + 8);

    // Recommendations
    println!("\n=== Optimization Recommendations ===");
    if size_of::<figment::Error>() > 64 {
        println!("⚠️  Box figment::Error ({} bytes)", size_of::<figment::Error>());
    }
    if size_of::<std::io::Error>() > 64 {
        println!("⚠️  Box std::io::Error ({} bytes)", size_of::<std::io::Error>());
    }
    if size_of::<serde_json::Error>() > 64 {
        println!("⚠️  Box serde_json::Error ({} bytes)", size_of::<serde_json::Error>());
    }
    ...
    if size_of::<InfernoError>() > target_size {
        ...
        println!("   Reduction needed: {} bytes", size_of::<InfernoError>() - target_size);
    } else {
...
fn measure_boxed_error_sizes() {
    println!("\n=== Boxed Error Sizes ===");
    println!("Box<figment::Error>: {} bytes", size_of::<Box<figment::Error>>());
    println!("Box<std::io::Error>: {} bytes", size_of::<Box<std::io::Error>>());
    println!("Box<serde_json::Error>: {} bytes", size_of::<Box<serde_json::Error>>());
