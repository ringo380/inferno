...
async fn main() -> Result<()> {
    ...
    let result = match cli.command {
        Commands::Run(args) => inferno_ai::cli::run::execute(args, &config).await,
        Commands::Batch(args) => inferno_ai::cli::batch::execute(args, &config).await,
        Commands::Serve(args) => inferno_ai::cli::serve::execute(args, &config).await,
        Commands::Models(args) => inferno_ai::cli::models::execute(args, &config).await,
        Commands::Metrics(args) => inferno_ai::cli::metrics::execute(args, &config).await,
        Commands::Bench(args) => inferno_ai::cli::bench::execute(args, &config).await,
        Commands::Validate(args) => inferno_ai::cli::validate::execute(args, &config).await,
        ...
        Commands::Audit(args) => inferno_ai::cli::audit::execute(args, &config).await,
        Commands::Queue(args) => inferno_ai::cli::batch_queue::execute(args, &config).await,
        Commands::Version(args) => inferno_ai::cli::versioning::execute(args, &config).await,
        Commands::Gpu(args) => inferno_ai::cli::gpu::execute(args, &config).await,
        ...
        Commands::MultiModal(args) => inferno_ai::cli::multimodal::handle_multimodal_command(args)
            .await
            .map_err(|e| anyhow::anyhow!(e)),
        Commands::Deployment(args) => {
            inferno_ai::cli::deployment::handle_deployment_command(args).await
        }
        Commands::Marketplace(args) => {
            inferno_ai::cli::marketplace::handle_marketplace_command(args).await
        }
        Commands::Package(args) => inferno_ai::cli::package::handle_package_command(args).await,

        // Simplified package manager aliases
        Commands::Install(args) => inferno_ai::cli::package::handle_install_simple(args).await,
        Commands::Remove(args) => inferno_ai::cli::package::handle_remove_simple(args).await,
        ...
        Commands::ApiGateway(args) => inferno_ai::cli::api_gateway::execute(args, &config).await,
        Commands::ModelVersioning(args) => {
            inferno_ai::cli::model_versioning::execute(args, &config).await
        }
        ...
        Commands::Upgrade(args) => inferno_ai::cli::upgrade::execute(args, &config).await,
        Commands::Tui => inferno_ai::tui::launch(&config).await,
    };
...
async fn init_background_update_service(config: &Config) -> Result<BackgroundUpdateService> {
    ...
    let upgrade_config = match inferno_ai::upgrade::config::UpgradeConfig::from_config(config) {
        Ok(config) => config,
        Err(e) => {
            warn!("Failed to load upgrade config, using defaults: {}", e);
            inferno_ai::upgrade::config::UpgradeConfig::default()
        }
    };
