...
impl UpgradeManager {
    ...
    pub async fn install_update(&self, update_info: &UpdateInfo) -> UpgradeResult<()> {
        info!(
            "Starting installation of version {}",
            ...
            Err(e) => {
                ...
                if let Err(rollback_error) = self.rollback_from_backup(&backup_path).await {
                    ...
                    // Update status with recovery available
                    {
                        ...
                        *status = UpgradeStatus::Failed {
                            error: format!(
                                "Installation failed: {}. System restored to previous version.",
                                e
                            ),
    ...
    async fn download_update(&self, update_info: &UpdateInfo) -> UpgradeResult<PathBuf> {
        ...
        self.emit_event(UpgradeEventType::DownloadStarted, "Starting download")
            .await;
        ...
        let progress_callback = move |bytes_downloaded: u64, total_bytes: u64, speed: u64| {
            ...
            let event = UpgradeEvent {
                ...
                message: format!(
                    ...
                    bytes_downloaded, total_bytes, progress
                ),
        ...
        match self
            .downloader
            .download_update(download_url, expected_checksum, progress_callback)
            .await
        {
            Ok(package_path) => {
    ...
    async fn create_backup(&self) -> UpgradeResult<PathBuf> {
        ...
        self.update_installation_status(InstallationStage::PreparingBackup, 0.0)
            .await;
    ...
    async fn perform_installation(
        ...
        self.update_installation_status(InstallationStage::VerifyingUpdate, 10.0)
            .await;
        self.safety_checker
            .read()
            .await
            .verify_package(package_path, update_info)
            .await?;

        // Stage 2: Prepare for upgrade
        self.update_installation_status(InstallationStage::StoppingServices, 20.0)
            .await;
        ...
        self.update_installation_status(InstallationStage::InstallingFiles, 40.0)
            .await;
        self.platform_handler
        ...
        self.update_installation_status(InstallationStage::UpdatingConfiguration, 70.0)
            .await;
        // Configuration updates would go here

        // Stage 5: Start services
        self.update_installation_status(InstallationStage::StartingServices, 80.0)
            .await;
        // Service restart would go here

        // Stage 6: Verify installation
        self.update_installation_status(InstallationStage::VerifyingInstallation, 90.0)
            .await;
        ...
        // Stage 7: Cleanup
        self.update_installation_status(InstallationStage::CleaningUp, 95.0)
            .await;
        self.platform_handler
        ...
        self.update_installation_status(InstallationStage::CleaningUp, 100.0)
            .await;
        Ok(())
    }
    ...
    async fn rollback_from_backup(&self, backup_path: &PathBuf) -> UpgradeResult<()> {
        ...
        self.emit_event(UpgradeEventType::RollbackStarted, "Starting rollback")
            .await;
        ...
        match self.backup_manager.restore_backup(backup_path).await {
            Ok(_) => {
                ...
                self.emit_event(UpgradeEventType::RollbackCompleted, "Rollback completed")
                    .await;
                Ok(())
            }
            Err(e) => {
    ...
    fn create_platform_handler(_config: &UpgradeConfig) -> Result<Box<dyn PlatformUpgradeHandler>> {
        #[cfg(target_os = "macos")]
        {
            Ok(Box::new(crate::upgrade::macos::MacOSUpgradeHandler::new(
                _config,
            )?))
        }
        ...
        #[cfg(target_os = "linux")]
        {
            Ok(Box::new(crate::upgrade::linux::LinuxUpgradeHandler::new(
                _config,
            )?))
        }
        ...
        {
            Ok(Box::new(
                crate::upgrade::windows::WindowsUpgradeHandler::new(_config)?,
            ))
        }
        ...
        {
            Err(anyhow::anyhow!(
                "Unsupported platform for seamless upgrades"
            ))
        }
    }
...
impl UpgradeManager {
    ...
    pub async fn detect_installation_context(&self) -> UpgradeResult<InstallationContext> {
        ...
        let previous_installation_info = self
            .detect_previous_installation(&installation_directory)
            .await?;
            ...
            Some(prev_version) => {
                if *prev_version == self.current_version {
                    ...
                    InstallationType::Downgrade {
                        from_version: prev_version.clone(),
                    }
                } else {
    ...
    pub async fn install_update_contextual(&self, update_info: &UpdateInfo) -> UpgradeResult<()> {
        ...
        match context.installation_type {
            InstallationType::FreshInstall => {
                info!(
                    "Performing fresh installation of version {}",
                    update_info.version.to_string()
                );
            ...
            InstallationType::Reinstall => {
                info!(
                    "Performing reinstallation of version {}",
                    update_info.version.to_string()
                );
    ...
    async fn detect_previous_installation(
        &self,
        install_dir: &PathBuf,
    ) -> UpgradeResult<Option<ApplicationVersion>> {
    ...
    async fn perform_fresh_install(
        &self,
        update_info: &UpdateInfo,
        ...
        self.platform_handler
            ...
            .await
            .map_err(|e| {
                UpgradeError::InstallationFailed(format!("Fresh install failed: {}", e))
            })?;

        // Create version tracking files
        self.create_installation_metadata(update_info, context)
            .await?;
    ...
    async fn perform_upgrade_install(
        &self,
        update_info: &UpdateInfo,
        context: &InstallationContext,
    ) -> UpgradeResult<()> {
        ...
        .await;
        let backup_id =
            self.backup_manager.create_backup().await.map_err(|e| {
                UpgradeError::BackupFailed(format!("Pre-upgrade backup failed: {}", e))
            })?;
        ...
        match self.platform_handler.install_update(&package_path).await {
            Ok(_) => {
                ...
                self.update_installation_metadata(update_info, context)
                    .await?;
    ...
    async fn perform_reinstall(
        ...
        self.emit_event(
            UpgradeEventType::InstallationStarted,
            "Creating safety backup for reinstall",
        )
        .await;
        let backup_id = self.backup_manager.create_backup().await.map_err(|e| {
            UpgradeError::BackupFailed(format!("Pre-reinstall backup failed: {}", e))
        })?;
    ...
    async fn perform_downgrade_install(
        &self,
        update_info: &UpdateInfo,
        context: &InstallationContext,
    ) -> UpgradeResult<()> {
        ...
        self.emit_event(
            UpgradeEventType::InstallationStarted,
            "Creating mandatory backup for downgrade",
        )
        .await;
        let backup_id = self.backup_manager.create_backup().await.map_err(|e| {
            UpgradeError::BackupFailed(format!("Pre-downgrade backup failed: {}", e))
        })?;
    ...
    async fn detect_version_from_binary(
        &self,
        install_dir: &PathBuf,
    ) -> UpgradeResult<ApplicationVersion> {
        ...
        Err(UpgradeError::Internal(
            "Could not detect version from binary".to_string(),
        ))
    }

    async fn detect_version_from_manifest(
        &self,
    ...
    async fn create_installation_metadata(
        &self,
        update_info: &UpdateInfo,
        context: &InstallationContext,
    ) -> UpgradeResult<()> {
        let version_file = context.installation_directory.join("VERSION");
        tokio::fs::write(&version_file, update_info.version.to_string())
            .await
            .map_err(|e| UpgradeError::Internal(format!("Failed to create version file: {}", e)))?;
        Ok(())
    }

    async fn update_installation_metadata(
        &self,
        update_info: &UpdateInfo,
        context: &InstallationContext,
    ) -> UpgradeResult<()> {
    ...
    async fn preserve_configuration(&self) -> UpgradeResult<PathBuf> {
        ...
        let temp_config =
            std::env::temp_dir().join(format!("inferno_config_backup_{}", Utc::now().timestamp()));
    ...
    async fn preserve_user_data(&self) -> UpgradeResult<PathBuf> {
        ...
        let temp_data =
            std::env::temp_dir().join(format!("inferno_data_backup_{}", Utc::now().timestamp()));
    ...
    async fn download_and_verify_update(&self, update_info: &UpdateInfo) -> UpgradeResult<PathBuf> {
        ...
        self.emit_event(UpgradeEventType::DownloadCompleted, "Download completed")
            .await;
...
mod tests {
    ...
    async fn test_event_subscription() {
        ...
        // Emit a test event
        manager
            .emit_event(UpgradeEventType::UpdateCheckStarted, "Test message")
            .await;
